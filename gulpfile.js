...
gulp.task('default', () => {
  // runSequence('browse-sync', () => {
  //   console.log("started");
  // });

  gulp.watch('src/**/*.scss', ['sass']);
  // gulp.watch(['src/**/*.ts', 'node_modules/**/lib/main.js'], ['typescript']);
  gulp.watch(['src/**/*.ts', 'node_modules/utility-collection/lib/main.js'], () => {
    runSequence(["typescript"], ["lib_js"], () => {});
  });

  gulp.watch('lib/main.css', ['lib_css']);
  // gulp.watch('lib/**/*.js', ['lib_js']);

  runSequence('browse-sync', () => { console.log("started");  });
  gulp.watch(["src/**/*.html", "dist/*.js"]).on('change', browserSync.reload);
});
// src => lib
gulp.task('sass', () => {
  return gulp
